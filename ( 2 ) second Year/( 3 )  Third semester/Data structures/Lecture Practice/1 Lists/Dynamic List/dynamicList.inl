...
template<class DataType>
DynamicList<DataType>::DynamicList(const DynamicList& other) {
...
template<class DataType>
DynamicList<DataType>& DynamicList<DataType>::operator=(const DynamicList& other){
    if(this != &other){
        this->size = other.getSize();
        this->capacity = other.getCapacity();
...
template<class T>
inline bool DynamicList<T>::contains(const T& element) const{
    for(size_t i = 0;i <size;++i){
...
template<class T>
inline size_t DynamicList<T>::getCapacity() const {return this->capacity;}
    // !new
template<class T>
...
template<class T>
inline void DynamicList<T>::ensureSize(size_t size_){
    if(size_ <= capacity) return;
    int tempVal = 1;
    while(tempVal < size_){
        tempVal *= 2;
    }
    if(tempVal != this->size){
        resize(tempVal);
    }
}
